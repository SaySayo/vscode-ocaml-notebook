...
let _notebook_controller =
  ...
  let handler ~(cells:NotebookCell.t list) ~notebook:_ ~controller =
    ...
    let () =
      ...
      cells
      |> List.map
        (fun (cell : NotebookCell.t) ->
          let open Promise.Syntax in
          let execution =
            NotebookController.createNotebookCellExecution controller
              ~cell
          in
          let () =
            NotebookCellExecution.set_executionOrder execution 0
          in
          ...
          let* () =
            NotebookCellExecution.replaceOutput execution ~out:notebook_cell_output ~cell
              ()
          in
          (* Call execution.end *)
          let now = (new%js Js.date_now)##getTime in
