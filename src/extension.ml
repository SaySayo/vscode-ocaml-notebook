    ...
    let handler ~cells ~notebook:_ ~controller = 
      ...
      let () = 
        ...
        List.iter (fun cell ->
          ...
          let execution = 
            Vscode.NotebookController.createNotebookCellExecution controller ~cell
          in
          let () = Vscode.NotebookCellExecution.set_executionOrder execution 0 in
          ...
          let notebook_cell_output_item = 
           ...
           Vscode.NotebookCellOutputItem.make ~data ~mime in
          (* Create CellOutput *)
          let notebook_cell_output = NotebookCellOutput.make ~items:[notebook_cell_output_item] () in
          (* Assign the cell output to the execution (replaceOutput) *)
          let* () = NotebookCellExecution.replaceOutput ~out:notebook_cell_output ~cell () in
          (* Call execution.end *)
          let now = Js.Date.make () |> Js.Date.getTime in
          let () = Vscode.NotebookCellExecution.end_ ~success:true ~endTime:now () in
          ()
        ) cells
      in
      Promise.return ()
